/*
 * Copyright Notice:
 * Original code Copyright (c) 2016-2022 PrimeTek
 * This code section is based on primereact.
 */
import {
  MouseEventHandler,
  forwardRef,
...
export const BasicScroll = forwardRef<
  BasicScrollMethods,
  BasicScrollProps
>((inProps, ref) => {
  const props = BasicScrollBase.getProps(inProps);
  ...
  const calculateContainerHeight = () => {
    ...
    const pureContainerHeight =
      getHeight(containerRef.current!) - parseInt(xBarStyles.height, 10);

    if (containerStyles.maxHeight !== 'none' && pureContainerHeight === 0) {
      if (
        contentRef.current!.offsetHeight + parseInt(xBarStyles.height, 10) >
  ...
  const moveBar = () => {
    ...
    const bottom =
      (containerRef.current!.clientHeight - xBarRef.current!.clientHeight) * -1;
    ...
    const right =
      (containerRef.current!.clientWidth - yBarRef.current!.clientWidth) * -1;
  ...
  const otherProps = BasicScrollBase.getOtherProps(props);
  const className = classNames(
    'scroll-container component clearfix',
    prefixCls,
    props.className ?? '',
  );
  ...
  return (
    ...
    >
      ...
      <div
        ref={xBarRef}
        className="scroll-container-bar scroll-container-bar-x"
        onMouseDown={onXBarMouseDown}
      />
      <div
        ref={yBarRef}
        className="scroll-container-bar scroll-container-bar-y"
